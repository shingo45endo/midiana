...
function makeYamahaGmParser(modelProp) {
	const {modelName, modelId, address} = modelProp;
		...
		case 0x30: // Bulk Dump Request
			...
			handler = ((modelId, modelName, commandName) => {
				...
				return (bytes) => {
					...
					const requiredSize = as21bitBE(...bytes.slice(index, index + 3));
					index += 3;
					...
					return {
						mfrId, deviceId, modelId, modelName, commandId, commandName, address, requiredSize, checkSum, expectedCheckSum, isCheckSumError,
