...
const modelProps = [
	...
	// [1984-??-?? (Ver.1.0)] Roland MSQ-100 (Digital Keyboard Recoder)
	{
		modelName: 'MSQ-100',
...
function makeRolandOldParsers(modelProp) {
	...
	for (const commandId of modelProp.commands) {
		...
		case 0x32:	// Tone Parameter Change
			key = `f0 ${mfrIdHex} ${commandHex} 00`;
			regexp = new RegExp(String.raw`^f0 ${mfrIdHex} ${commandHex} 0. .. (?:.. )+f7$`, 'u');
			handler = ((modelName, commandName) => {
				return (bytes) => {
					...
					return {
						mfrId, mfrIdHex, commandId, commandName, deviceId, modelName, params, rawValues,
						_idKey: mfrIdHex, _paramKeys: ['params'], _valueKeys: ['rawValues'],
					};
				};
			})(modelProp.modelName, commandNames[commandId]);
			break;

		case 0x34:	// Program Number (PGR)
		case 0x36:	// Individual Tone/Patch Parameter (IPR)
		case 0x37:	// Bulk Dump (BLD)
			key = `f0 ${mfrIdHex} ${commandHex} 00 ${modelIdHex}`;
			regexp = new RegExp(modelProp?.reStrs?.[commandId] ?? String.raw`^f0 ${mfrIdHex} ${commandHex} 0. ${modelIdHex} .. .. .. (?:.. )+f7$`, 'u');
			handler = ((modelIdHex, modelName, commandName) => {
				return (bytes) => {
					...
					return {
						mfrId, mfrIdHex, commandId, commandName, deviceId, modelId, modelIdHex, modelName, params, rawValues,
						_idKey: [mfrIdHex, modelIdHex].join(' '), _paramKeys: ['params'], _valueKeys: ['rawValues'],
					};
				};
		...
		case 0x42:	// Data (DAT)
			key = `f0 ${mfrIdHex} ${commandHex} 00 ${modelIdHex}`;
			regexp = new RegExp(String.raw`^f0 ${mfrIdHex} ${commandHex} 0. ${modelIdHex} (?:.. )+.. f7$`, 'u');
			handler = ((modelIdHex, modelName, commandName) => {
				return (bytes) => {
		...
		case 0x71:	// Communication Error (ERR)
			key = `f0 ${mfrIdHex} ${commandHex}`;
			regexp = new RegExp(String.raw`^f0 ${mfrIdHex} ${commandHex} f7$`, 'u');
			handler = ((modelName, commandName) => {
				return (bytes) => {
		...
		case 0x57:	// Data (7-8 Conversion)	// TODO: Support conversion
			key = `f0 ${mfrIdHex} ${commandHex} ${modelIdHex}`;
			regexp = new RegExp(String.raw`^f0 ${mfrIdHex} ${commandHex} ${modelIdHex} .. (?:.. )+.. f7$`, 'u');
			handler = ((modelIdHex, modelName, commandName) => {
				return (bytes) => {
					...
					return {
						mfrId, mfrIdHex, modelId, modelIdHex, modelName, commandId, commandName, params, rawValues, checkSum, expectedCheckSum, isCheckSumError,
						_idKey: [mfrIdHex, modelIdHex].join(' '), _paramKeys: ['params'], _valueKeys: ['rawValues'],
					};
				};
			})(modelIdHex, modelProp.modelName, commandNames[commandId]);
		...
		if (modelProps.length > 1) {
			...
			for (const command of [...new Set(modelProps.map((e) => e.commands).flat())]) {
				...
				if (modelProps.every((modelProp) => modelProp.reStrs?.[command])) {
					parsers.push(...makeRolandOldParsers({modelId,
						modelName: `${modelName}?`,
						commands: [command],
					}));
				}
