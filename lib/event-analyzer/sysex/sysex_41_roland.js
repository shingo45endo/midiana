...
//	console.assert(/^(?:(?:00 )*[0-7][0-9a-f]|01 [0-7][0-9a-f] [0-7][0-9a-f])$/u.test(modelIdHex));
	...
	for (const commandId of modelProp.commands) {
		...
		case 0x40:	// WSD
		case 0x41:	// RQD
			regexp = new RegExp(String.raw`^f0 ${mfrIdHex} ${deviceIdReStr} ${modelIdHex} ${commandHex} ${addrReStr} ${[...new Array(addrLen)].fill('..').join(' ')} .. f7$`, 'u');
		...
		case 0x12:	// DT1
		case 0x42:	// DAT
			regexp = new RegExp(String.raw`^f0 ${mfrIdHex} ${deviceIdReStr} ${modelIdHex} ${commandHex} ${addrReStr} (?:.. )+.. f7$`, 'u');
			handler = ((modelIdHex, modelIdLen, modelName, commandName, addrLen) => {
				const idKey = [mfrIdHex, modelIdHex].join(' ');
				return (bytes) => {
					let index = 0;
		...
		case 0x4e:	// ERR
		case 0x4f:	// RJC
			regexp = new RegExp(String.raw`^f0 ${mfrIdHex} ${deviceIdReStr} ${modelIdHex} ${commandHex} f7$`, 'u');
		...
		for (const deviceIdHex of deviceIdHexs) {
			const key = `f0 ${mfrIdHex} ${deviceIdHex} ${modelIdHex} ${commandHex}`;
