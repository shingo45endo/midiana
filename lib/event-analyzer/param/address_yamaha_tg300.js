...
import json from './address_43_2b_tg300.json' with {type: 'json'};
...
const additionalParsers = [
    ...(new Array(48)).fill().map((_, begin) => {
        ...
        return sizes.map((size) => {
            const BARS_WIDTH_HEIGHT = 720;
            const INTERVAL = BARS_WIDTH_HEIGHT / 16;
            const GAP = INTERVAL * 0.01;
            const DOT_WIDTH_HEIGHT = INTERVAL - GAP;
            ...
            const barRectsSvgs = [...new Array(48)].map((_, i) => {
                ...
                return (new Array(Math.min(7, 16 - baseX))).fill().map((_, j) => {
                    ...
                    const y = INTERVAL * baseY;
                    return `<rect id="dot-${baseX + j + baseY * 16}" x="${x}" y="${y}" width="${DOT_WIDTH_HEIGHT}" height="${DOT_WIDTH_HEIGHT}" />`;
                }).join('');
            });

            return {
                regexp: makeAddressRegexp(`07 01 ${begin.toString(16).padStart(2, '0')}`),	// Bitmap Window
                size,
                replacer: {
                    formatter: (...values) => {
                    ...
                    renderer: (...values) => {
                        ...
                        return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${BARS_WIDTH_HEIGHT} ${BARS_WIDTH_HEIGHT}"><style type="text/css">rect {opacity: ${OPACITY_OFF}} ${idStr} {opacity: ${OPACITY_ON}}</style><g fill="currentColor">${barRectsSvgs.filter((e, i) => isInRanges[i]).join('')}</g></svg>`;
                    },
                },
            };
        });
