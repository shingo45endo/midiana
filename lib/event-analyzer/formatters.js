...
export function makeFormatterFromTable(table) {
	if (Array.isArray(table)) {
		return ((table) => {
			console.assert(table.every((e) => (typeof e === 'string')));
			return (value) => {
		...
		return ((table) => {
			console.assert(Object.values(table).every((e) => (typeof e === 'string')));
			return (value) => table[value] ?? String(value);
		})(table);
	}
}
	...
	return (resolution, format) => {
		...
		const formatter = ((resolution, format) => {
			...
			return (value) => {
				...
				const vStr = (!Number.isInteger(resolution) || (vStr2.length < vStr1.length && /[\d.-]+/u.test(vStr2))) ? vStr2 : vStr1;
				const ret = vStr.replace(/0{2,}$/u, '0');
